    ...
    if split_name == "coco_test2014":
      ...
      with open("/data/visualgenome/image_data.json") as f:
        for item in json.load(f):
          ...
          filepath = os.path.join("/data/visualgenome/",
                                  item["url"].split("rak248/")[-1])
          split.append((filepath, image_id))
    elif split_name == "vietnamese":
        with open("/dataset/Dataset/Vietnamese-COCO/annotations/captions_COCO5k_138test_vntk_HumanScore_4cap_GT.json", encoding="utf-8") as f:
          data = json.load(f)
        for item in data["images"]:
            image_id = int(item["id"])
            filepath = os.path.join(
                "/dataset/Dataset/Vietnamese-COCO/138_Test_Images", item["file_name"])
            split.append((filepath, image_id))
    elif split_name == "uit-viic":
        with open("/dataset/Dataset/UIT-ViIC/train/annotations/uitviic_captions_train2017.json", encoding="utf-8") as f:
            data = json.load(f)
        for item in data["images"]:
            image_id = int(item["id"])
            filepath = os.path.join(
                "/dataset/Dataset/UIT-ViIC/train/images", item["file_name"])
            split.append((filepath, image_id))
    elif split_name == "merge-data":
        file_path_list = os.listdir('/dataset/Dataset/Merge_data/image/')
        count_id = 0
        for file_path in file_path_list:
            image_id = count_id
            filepath = os.path.join(
                '/dataset/Dataset/Merge_data/image/', file_path)
            split.append((filepath, image_id))
            count_id += 1
    else:
      print("Unknown split")
...
def get_detections_from_im(net, im_file, image_id, conf_thresh=0.2):
    ...
    for i in range(len(keep_boxes)):
        labels.append({
            "class": "{}".format(classes[objects[i]+1].capitalize()),
            "rect": "{}".format(cls_boxes[keep_boxes][i].tolist()),
            "conf": "{}".format(max_conf[keep_boxes][i])
        })
...
def generate_tsv(gpu_id, prototxt, weights, image_ids, outfile):
    ...
    if os.path.exists(outfile):
        with open(outfile) as tsvfile, open("train.label.tsv") as labeltsvfile:
            reader = csv.DictReader(
                tsvfile, delimiter="\t", fieldnames=FIELDNAMES)
            Labelreader = csv.DictReader(labeltsvfile, delimiter="\t")
            for item in reader:
                found_ids.add(int(item["image_id"]))
    missing = wanted_ids - found_ids
    if len(missing) == 0:
        print(("GPU {:d}: already completed {:d}".format(
            gpu_id, len(image_ids))))
    else:
        print(("GPU {:d}: missing {:d}/{:d}".format(gpu_id,
                                                   len(missing), len(image_ids))))
    if len(missing) > 0:
        caffe.set_mode_gpu()
        caffe.set_device(gpu_id)
        net = caffe.Net(prototxt, caffe.TEST, weights=weights)
        with open(outfile, "at") as tsvfile, open("train.label.tsv", "at") as labeltsvfile:
            writer = csv.DictWriter(
                tsvfile, delimiter="\t", fieldnames=FIELDNAMES)
            Labelwriter = csv.DictWriter(
                labeltsvfile, delimiter="\t", fieldnames=LABELNAMES)
            _t = {"misc": Timer()}
            count = 0
...
def merge_tsvs():
    # test = ["/workingspace/bottom-up-attention/uitviic_resnet101_faster_rcnn_genome_train.tsv.%d" %
    #         i for i in range(1, 3)]
    test = ["/workingspace/bottom-up-attention/merged_training_captioning_train.tsv.%d" %
            i for i in range(4, 8)]

    outfile = "train.tsv"
    with open(outfile, "at") as tsvfile:
